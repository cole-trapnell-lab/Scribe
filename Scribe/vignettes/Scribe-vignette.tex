...
\hlkwd{data}\hlstd{(}\hlstr{"neuron_network"}\hlstd{)} \hlcom{# network data}
\hlkwd{data}\hlstd{(}\hlstr{"neuron_sim_cds"}\hlstd{)} \hlcom{# network data}
\hlkwd{data}\hlstd{(}\hlstr{"na_sim_cds"}\hlstd{)} \hlcom{# network data}
\hlkwd{data}\hlstd{(}\hlstr{"example_graph"}\hlstd{)} \hlcom{# network data}
...
\hlstd{rdi_crdi_pseudotime_res_list} \hlkwb{<-} \hlkwd{calculate_temporal_causality}\hlstd{(}\hlkwc{cds_subset} \hlstd{= na_sim_cds[}\hlnum{1}\hlopt{:}\hlnum{12}\hlstd{,} \hlnum{1}\hlopt{:}\hlnum{200}\hlstd{],} \hlkwc{window_size} \hlstd{=} \hlnum{50}\hlstd{)} \hlcom{#13 mature gives Na values}
...
{\centering \includegraphics[width=\maxwidth]{figure/plot_temporal_rdi-1} 
...
{\centering \includegraphics[width=\maxwidth]{figure/plot_temporal_rdi-2} 
...
{\centering \includegraphics[width=\maxwidth]{figure/plot_temporal_rdi-3} 
...
{\centering \includegraphics[width=\maxwidth]{figure/plot_temporal_rdi-4} 
...
\hlstd{rdi_list} \hlkwb{<-} \hlkwd{calculate_rdi_multiple_run}\hlstd{(neuron_sim_cds[,} \hlnum{1}\hlopt{:}\hlnum{200}\hlstd{],} \hlkwc{run_vec} \hlstd{=} \hlkwa{NULL}\hlstd{,} \hlkwc{delay} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{),} \hlkwc{super_graph} \hlstd{=} \hlkwd{as.matrix}\hlstd{(super_graph),} \hlkwc{method} \hlstd{=} \hlnum{1}\hlstd{)} \hlcom{#* 100 + noise}

\hlkwd{dimnames}\hlstd{(rdi_list}\hlopt{$}\hlstd{max_rdi_value)} \hlkwb{<-} \hlkwd{list}\hlstd{(gene_name_vec, gene_name_vec)}
\hlstd{con_rdi_res_test} \hlkwb{<-} \hlkwd{calculate_conditioned_rdi_multiple_run}\hlstd{(neuron_sim_cds[,} \hlnum{1}\hlopt{:}\hlnum{200}\hlstd{],} \hlkwc{run_vec} \hlstd{=} \hlkwa{NULL}\hlstd{,} \hlkwc{super_graph} \hlstd{=} \hlkwd{as.matrix}\hlstd{(super_graph),} \hlkwc{rdi_list} \hlstd{= rdi_list,} \hlkwc{top_incoming_k} \hlstd{=} \hlnum{1}\hlstd{)}
...
\hlkwd{data}\hlstd{(}\hlstr{"example_graph"}\hlstd{)} \hlcom{#load the example_graph (we need to mannually set the initial_nodes 1:2 in .process_graph1 function)}

\hlkwd{plot_causal_network}\hlstd{(example_graph}\hlopt{$}\hlstd{g,} \hlkwc{type} \hlstd{=} \hlstr{'igraph'}\hlstd{,} \hlkwc{layout} \hlstd{= layout_in_circle)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/plot_network-3} 

}


\begin{kframe}\begin{alltt}
\hlcom{# plot_network(example_graph$g, type = 'hiearchy')}
\hlkwd{plot.netbiov}\hlstd{(example_graph)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/plot_network-4} 

}


\begin{kframe}\begin{alltt}
\hlkwd{plot_causal_network}\hlstd{(example_graph}\hlopt{$}\hlstd{g,} \hlkwc{type} \hlstd{=} \hlstr{'arcdiagram'}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/plot_network-5} 
